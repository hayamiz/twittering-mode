...
(defun twittering-normalize-proxy-vars ()
  "Normalize the type of `twittering-http-proxy-port' and `twittering-https-proxy-port'."
  (mapc (lambda (sym)
...
(defun twittering-make-http-request (method header-list host port path query-parameters post-body use-ssl)
  "Return an alist specifying a HTTP request.
  ...
  (let* ((request-token-alist
	...
	(let* ((str-height (length (split-string str "\n")))
	  ...
	  (if twittering-oauth-invoke-browser
	    ...
	    (when (y-or-n-p "Open authorization URL with browser? ")
	      (browse-url authorize-url)))
	  (let* ((pin
...
(defun twittering-timeline-spec-is-direct-message-events-p (spec)
  "Return non-nil if SPEC is a timeline spec which is related of direct_message_events."
  (and spec
       (memq (car spec) '(direct_message_events))))

(defun twittering-timeline-spec-is-direct-messages-p (spec)
  "Return non-nil if SPEC is a timeline spec which is related of direct_messages."
  (and spec
...
(defun twittering-toggle-show-replied-statuses ()
  ...
  (if (twittering-replied-statuses-visible-p)
      (twittering-hide-replied-statuses (called-interactively-p 'interactive))
